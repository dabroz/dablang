...
const llvm::Type * qneu_PointerType::llvm()
{
	const Type * basetype = base->llvm();
	if (!basetype)
	{
		basetype = base->llvm();
	}
...
const llvm::Type * qneu_PrimitiveType::llvm()
{
...
const llvm::Type * qneu_StructType::llvm()
{
	...
	{
		const llvm::Type * t = members[i].type->llvm();	
		if (!t)
		{
	...
	const llvm::StructType * t = llvm::StructType::get(llvm::getGlobalContext(), params, false);

	TheModule->getTypeSymbolTable().insert(this->name(), t);

	cachetype = t;
	return t;
}

const llvm::Type * qneu_VoidType::llvm()
{
...
Value * qneu_StructType::getLllvVariable( qString name, Value * parent )
{
	...
	return Builder.CreateGEP(parent, gepp, gepp + 2);
}
...
const llvm::Type * qneu_ArrayType::llvm()
{
